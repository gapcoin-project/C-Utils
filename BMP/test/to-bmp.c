...
int main(int argc, char *argv[]) {
  ...
  uint32_t bmp_size = (uint32_t) sqrt((double) f_size);

  BMP bmp;
  init_RGB_BMPHeader(&bmp.head, bmp_size / 3, bmp_size);

  char buffer[256];
  sprintf(buffer, "%s.bmp", argv[1]);
  ...
  if(write(fd, f_data, bmp_size * bmp_size) == -1) {
